...
import { LogOut, User, ChevronDown, X, Save, Phone, MapPin, HeartPulse, Mail } from "lucide-react";
...
const SettingField = ({ label, name, value, onChange, icon: Icon, disabled = false }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border transition-colors ${
      ...
      {Icon && <Icon className={`w-4 h-4 ${disabled ? "text-slate-400" : "text-indigo-500"}`} />}
      <input 
        type="text" 
        name={name}
        value={value || ""} 
      ...
      />
      {disabled && <span className="text-[10px] font-bold px-1.5 py-0.5 bg-slate-200 dark:bg-slate-700 rounded text-slate-500">LOCKED</span>}
    </div>
  </div>
);
...
export default function PatientPortal() {
  ...
  function getDefaultPatient() {
    return {
      ...
      phone: "—", emergencyContact: { name: "", relation: "", phone: "" }, address: "—",
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-gradient-to-b from-[#e8efff] to-[#e6eefc] dark:from-slate-950 dark:to-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-500 overflow-hidden">
      ...
      <div className="flex-none h-16 px-6 flex items-center justify-between border-b border-white/20 dark:border-white/5 bg-white/30 dark:bg-slate-900/30 backdrop-blur-md z-40 relative">
        <div className="flex items-center gap-8">
          <div className="font-black text-xl text-indigo-600 dark:text-indigo-400 tracking-tight flex items-center gap-2">
            <span className="bg-indigo-600 dark:bg-indigo-500 text-white w-8 h-8 rounded-lg flex items-center justify-center text-sm shadow-md">MV</span>
            MediVault
          </div>
        ...
        <div className="flex items-center gap-5">
          <div className="bg-white/50 dark:bg-black/40 backdrop-blur-md rounded-full p-1.5 border border-white/20 shadow-sm cursor-pointer hover:scale-105 transition-transform" onClick={toggleTheme}>
              <Classic duration={750} toggled={isDark} className="text-slate-600 dark:text-yellow-400 text-xl" />
          </div>
          ...
          <div className="relative" ref={menuRef}>
            ...
            >
              <div className="text-right hidden sm:block">
                <div className="text-sm font-bold text-slate-900 dark:text-white leading-none">{patient.name.split(' ')[0]}</div>
                <div className="text-[10px] uppercase font-bold tracking-wider text-slate-500 dark:text-slate-400 mt-0.5">{patient.uid}</div>
              </div>
              <img src={patient.profilePic || DEFAULT_AVATAR} alt="avatar" className="w-9 h-9 rounded-lg object-cover border-2 border-white dark:border-slate-600 shadow-sm" />
              ...
              {isMenuOpen && (
                ...
                >
                  <div className="px-3 py-2 border-b border-slate-200 dark:border-slate-700/50 mb-1">
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Signed in as</p>
                    <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{patient.email}</p>
                  </div>
                  
                  <button onClick={openSettings} className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-indigo-50 dark:hover:bg-white/5 rounded-lg transition-colors text-left">
                    <User className="w-4 h-4" /> Profile Settings
                  </button>

                  <div className="h-px bg-slate-200 dark:bg-slate-700/50 my-1" />

                  <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-sm font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors text-left">
      ...
      <div className="flex-1 p-4 min-h-0 relative z-0">
        <div style={{ display: "grid", gridTemplateColumns: "360px 1fr 360px", gap: 18, height: "calc(100vh - 100px)" }}>
          ...
          <>
            ...
            >
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4 mb-2">
                    ...
                    <span className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">Identity (Read Only)</span>
                    <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800"></div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4 mb-2">
                    <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800"></div>
                    <span className="text-[10px] font-bold uppercase text-indigo-500 tracking-widest">Contact Details</span>
                    <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800"></div>
                  </div>
              ...
              <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex justify-end gap-3">
                <button onClick={() => setIsSettingsOpen(false)} className="px-4 py-2 text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors">Cancel</button>
                <button onClick={saveProfile} className="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-lg shadow-indigo-500/30 flex items-center gap-2 transition-all active:scale-95"><Save className="w-4 h-4" /> Save Changes</button>
              </div>
            </motion.div>
          </>
        )}
